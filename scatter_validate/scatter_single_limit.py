...
class pic:
    ...
    def select(self):
        sql = "select s.trade_date,s.stock_id,s.high_price,s.low_price,s.increase,l.grade,l.monitor,l.type from stock_trade_data s " \
              " LEFT JOIN limit_up_single l " \
              " ON s.trade_code = l.trade_code " \
              " where s.trade_date >='{0}' and s.trade_date <='{1}' and l.type = 'standard'".format(self.start_date,self.end_date)
    ...
    def deal_data(self):
        ...
        self.monitor_df = self.sel_df[(self.sel_df.monitor == 1) & (self.sel_df.inc_1 >=2.5)]
    ...
    def show_pic(self):
        ...
        self.n_monitor_df.plot.scatter(x='grade', y='inc_2')
        plt.show()
if __name__ == '__main__':
    p =pic('2021-06-20','2021-10-31')
    p.show_pic()
